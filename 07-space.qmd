...
This aspect of R is covered in detail in the book [Geocomputation With R](https://geocompr.robinlovelace.net/index.html).
...
1. Plot only the geometry column of `crashes_sf` (**Hint:** the solution may contain `$geometry`). If the result is like that in @fig-crashes-sf-ex-1, congratulations, it worked!
...
```{r}
#| label: fig-crashes-sf-ex
#| layout-ncol: 3
#| fig-cap: "The `crashes_sf` dataset shown in map form with the function `plot()`."
...
This is illustrated below with reference to `zones`, a spatial object representing the Isle of Wight, that we created in the previous section.
**Exercises:**
...
1. Plot the geometry of the zones, with the zones containing crashes overlaid on top in red (see @fig-sp-ex-1).
2. Plot the zone containing the 2^nd^ crash in blue (see @fig-sp-ex-2).
3. **Bonus:** Plot all zones that intersect with a zone containing crashes, with the actual crash points plotted in black (see @fig-sp-ex-3).

```{r}
#| label: fig-sp-ex
#| layout-ncol: 3
#| fig-cap: "Illustration of the results of spatial subsetting."
#| message: false
#| warning: false
plot(zones$geometry)
...
1. Plot the `casualty_age` variable of the new `zones_joined` object (see @fig-joinf-1, to verify the result).
2. How many zones are returned in the previous command?
3. Select only the `geo_code` column from the `zones` and the `dark` column from `crashes_sf` and use the `left = FALSE` argument to return only zones in which crashes occurred. Plot the result. (**Hint:** it should look like the map shown in @fig-joinf-2)

See [Chapter 4](https://geocompr.robinlovelace.net/spatial-operations.html#spatial-joining) of *Geocomputation with R* for further information on geographic joins.

```{r}
#| label: fig-joinf
#| layout-ncol: 2
#| fig-cap: "Illustration of geographic joins."
#| message: false
...
That's fine for basic visualisation, but for publication-quality maps we recommend using `tmap`. See Chapter 8 of *Geocomputation with R* for further explanation and alternatives.
...
1. Create the plots of the `zones` object using `plot()` and `tm_shape() + tm_polygons()` functions (see @fig-plot3-1).
...
#| label: fig-plot3
#| layout-ncol: 2
#| fig-cap: "Illustration of the plot and tmap approaches for creating maps."
...
1. Download and plot all crashes reported in Great Britain in 2023. (**Hint:** see [the stats19 vignette](https://docs.ropensci.org/stats19/articles/stats19.html))
...
**Note:** You can also try the more recently developed [`osmactive` package](https://nptscot.github.io/osmactive/) on GitHub.


```{r}
#| label: fig-iowbuff
#| layout-ncol: 2
#| fig-cap: "Maps of the Isle of Wight."
#| message: false
#| echo: false
...
# Or nptscot/osmactive:
remotes::install_github("nptscot/osmactive") # alternative package for osm data
library(osmextract)
...
```{r}
#| label: fig-ex
#| fig-cap: "Roads in Essex downloaded with the code shown above."
#| echo: false
