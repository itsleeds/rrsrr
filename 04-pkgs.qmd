...
R has over 20k+ packages published on the official 'CRAN' site and many more published on code-sharing sites such as GitHub, as outlined in @sec-intro.
...
This chapter demonstrates the package lifecycle with reference to `{stats19}` and provides a taster of R's visualisation capabilities for general-purpose packages `{ggplot2}` and `{dplyr}`.
...
Packages can be updated with the command `update.packages()` or in 'Tools > Check for Package Updates' in RStudio.
...
```
**Exercises:**
1. Install a package that builds on `{ggplot2}` that begins with `gg`. Hint: enter `install.packages(gg)` and hit `Tab` when your cursor is between the `g` and the `)`.
...
We will learn more about this package and its other functions in @sec-data.