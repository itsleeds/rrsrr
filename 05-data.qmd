# Manipulating data {#sec-data}

This section is an introduction to manipulating datasets using the `{dplyr}` package.
As outlined in the previous section, `{dplyr}` and `{ggplot2}` are part of the `{tidyverse}`, which aims to provide a user-friendly framework for data science.
...
However, if you do not like this style of R code or you are simply curious, we encourage you to try alternative approaches for achieving the similar results using base R, the `{data.table}` R package or other languages such as [Python](https://www.python.org/) or [Julia](https://julialang.org/).
...
@sec-basics introduced R's basic syntax; @sec-rstudio showed how to use the Source Editor and other features of RStudio to support data science; and @sec-pkgs introduced the concept and practicalities of R packages, with reference to `{stats19}`, `{ggplot2}` and `{dplyr}`.

In this section, we will start with a blank slate.
In @sec-basics we learned that in R having a 'clear desk' means an *empty global environment*.
...
Although the data processing techniques in R are capable of handling large datasets, such as the `crashes_2020` object that we created in the previous section, representing 100k+ casualties, it makes sense to start small.
Let's start by re-creating the `crashes` dataset from Section 2, but this time using the `{tidyverse}` `tibble()` function. This is the `{tidyverse}` equivalent of base R's `data.frame`.
...
For further information, check out up-to-date online courses from respected organisations like [Data Carpentry](https://datacarpentry.org/R-ecology-lesson/index.html) and the free online [books](https://bookdown.org/) such as [R for Data Science](https://r4ds.had.co.nz/).
